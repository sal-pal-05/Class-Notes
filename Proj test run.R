...
# initializes and set up variables
count=1
count2=3
...
##indexing with for loop
 for(i in 1:14){
  ...
  ##creating tables "var" for every species along with respective colony count (also in for loop)
  var= ddply(new.df, .(Concentration, Colony_count), summarize,
        Species = (label))
  print(var)
  # creates and saves file for each graph plotted
...
graph2= ggplot(data=mean_CC2, aes(x= Species, y=mean_c_c)) + 
  geom_col() +
  ggtitle("Average Colony Count Across the 3 Concentrations")

graph2
mean_of_colony_count = mean_CC2$mean_c_c
ggplot(data=mean_CC2, aes(mean_of_colony_count)) + geom_histogram()
...
{
  if (y != 0){
    percentage <- c(percentage, ((Mean[1]/y) * x)) # change number in Mean[] to get different percentage
    print(percentage)
  }
}
